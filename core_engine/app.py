...
def nti_run():
    ...
    # Legacy gauntlet contract: tests read data["nii"]["nii_score"] directly.
    # Keep the signal envelope, but mirror the legacy score fields onto the
    # top level of the nii object instead of forcing callers into detail.
    if isinstance(nii, dict):
        nii["nii_score"] = nii_detail.get("nii_score")
        nii["nii_raw"] = nii_detail.get("nii_raw")
        nii["nii_label"] = nii_detail.get("nii_label")
