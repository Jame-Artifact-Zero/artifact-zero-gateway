...
def _q(conn, sql, params=None):
    """Execute a native PG query and return a psycopg2 cursor."""
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(sql, params)
    return cur

def _one(conn, sql, params=None):
    return _q(conn, sql, params).fetchone()

def _all(conn, sql, params=None):
    return _q(conn, sql, params).fetchall()
...
def config_get(key, default=""):
    try:
        ...
        row = _one(conn, "SELECT value FROM cockpit_config WHERE key=%s", (key,))
        conn.close()
...
def config_set(key, value):
    ...
    _run(conn, """INSERT INTO cockpit_config (key, value, updated_at) VALUES (%s, %s, %s)
...
def track_request(app):
    ...
    def _after(response):
        try:
            ...
            _run(conn, "INSERT INTO page_views (id,created_at,path,method,ip,user_agent,referrer,session_id,latency_ms) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
...
def log_nti_run(request_id, ip, text, result, latency_ms, session_id=""):
    try:
        ...
        conn = analytics_db()
        if conn is None: return
        _run(conn, "INSERT INTO nti_runs (id,created_at,ip,input_preview,word_count,nii_score,dominance,tilt_tags,latency_ms,session_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def log_relay_event(event_type, ip="", username="", detail=""):
    try:
        conn = analytics_db()
        if conn is None: return
        _run(conn, "INSERT INTO relay_events (id,created_at,event_type,ip,username,detail) VALUES (%s,%s,%s,%s,%s,%s)",
...
def api_config_dump():
    if not _is_admin(): return jsonify({"error":"Unauthorized"}), 401
    conn = analytics_db()
    rows = _all(conn, "SELECT key,value,updated_at FROM cockpit_config ORDER BY key")
    conn.close()
...
def _build_insights(conn):
    ...
    active = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s", (hour_ago,)) or {}).get("c", 0)
    ...
    today_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s", (today_start,)) or {}).get("c", 0)
    yest_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s AND created_at < %s", (yesterday_start, today_start)) or {}).get("c", 0)
    if yest_v > 0:
    ...
    active_pages = _all(conn,
        "SELECT path, COUNT(DISTINCT ip) as v FROM page_views WHERE created_at >= %s AND path NOT LIKE '/api/%%' AND path NOT LIKE '/static/%%' GROUP BY path ORDER BY v DESC LIMIT 5",
        (week_start,))
    if active_pages:
        page_summary = ", ".join(f"{r['path']} ({r['v']})" for r in active_pages)
        insights.append(("📄", f"Top pages this week: {page_summary}."))

    sc_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE path = '/safecheck' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    home_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE path = '/' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    ...
    f500_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE path LIKE '/fortune500%%' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    vc_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE path LIKE '/vc-funds%%' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    if f500_v or vc_v:
        insights.append(("🏆", f"Scoreboards: Fortune 500 {f500_v} visitors, VC Funds {vc_v} visitors this week."))

    signup_v = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE path = '/signup' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    if signup_v:
        insights.append(("👤", f"{signup_v} visitor{'s' if signup_v!=1 else ''} reached /signup this week."))

    nti_today = (_one(conn, "SELECT COUNT(*) as c FROM nti_runs WHERE created_at >= %s", (today_start,)) or {}).get("c", 0)
    nti_week = (_one(conn, "SELECT COUNT(*) as c FROM nti_runs WHERE created_at >= %s", (week_start,)) or {}).get("c", 0)
    ...
    avg_row = _one(conn, "SELECT AVG(nii_score) as a FROM nti_runs WHERE created_at >= %s AND nii_score IS NOT NULL", (week_start,))
    avg_nii = (avg_row or {}).get("a")
    if avg_nii is not None:
        d = avg_nii if avg_nii > 1 else avg_nii * 100
        insights.append(("🎯", f"Average NII this week: {d:.0f}."))

    refs = _all(conn, "SELECT referrer, COUNT(DISTINCT ip) as v FROM page_views WHERE referrer!='' AND referrer IS NOT NULL AND created_at >= %s GROUP BY referrer ORDER BY v DESC LIMIT 3", (week_start,))
    if refs:
        insights.append(("🔗", "Top sources: " + ", ".join(f"{r['referrer'][:40]} ({r['v']})" for r in refs)))

    slow = (_one(conn, "SELECT COUNT(*) as c FROM page_views WHERE latency_ms > 5000 AND created_at >= %s", (week_start,)) or {}).get("c", 0)
    if slow > 3:
        insights.append(("⚠️", f"{slow} slow requests (>5s) this week."))

    signups = (_one(conn, "SELECT COUNT(*) as c FROM relay_events WHERE event_type='signup' AND created_at >= %s", (week_start,)) or {}).get("c", 0)
...
def cockpit():
    ...
    active_now = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s", (ha,)) or {}).get("c", 0)
    today_views = (_one(conn, "SELECT COUNT(*) as c FROM page_views WHERE created_at >= %s", (ts,)) or {}).get("c", 0)
    today_visitors = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s", (ts,)) or {}).get("c", 0)
    week_views = (_one(conn, "SELECT COUNT(*) as c FROM page_views WHERE created_at >= %s", (ws,)) or {}).get("c", 0)
    total_nti = (_one(conn, "SELECT COUNT(*) as c FROM nti_runs") or {}).get("c", 0)
    nti_today = (_one(conn, "SELECT COUNT(*) as c FROM nti_runs WHERE created_at >= %s", (ts,)) or {}).get("c", 0)
    ...
    recent_visitors = _all(conn, """SELECT ip, COUNT(*) as hits, COUNT(DISTINCT path) as pages, string_agg(DISTINCT path, ',') as paths, MAX(created_at) as last_seen, MAX(referrer) as ref FROM page_views WHERE created_at >= %s GROUP BY ip ORDER BY last_seen DESC LIMIT 20""", ((now-timedelta(hours=24)).isoformat(),))
    recent_nti = _all(conn, "SELECT * FROM nti_runs ORDER BY created_at DESC LIMIT 15")
    pages_today = _all(conn, "SELECT path, COUNT(*) as hits, COUNT(DISTINCT ip) as visitors FROM page_views WHERE created_at >= %s GROUP BY path ORDER BY hits DESC LIMIT 15", (ts,))
    hourly = _all(conn, "SELECT to_char(created_at AT TIME ZONE 'UTC', 'HH24') || ':00' as hour, COUNT(*) as hits, COUNT(DISTINCT ip) as v FROM page_views WHERE created_at >= %s GROUP BY 1 ORDER BY 1 DESC", ((now-timedelta(hours=12)).isoformat(),))
    conn.close()
...
def cockpit_visitor(ip):
    ...
    conn = analytics_db()
    raw_rows = _all(conn,
        "SELECT created_at, path, method, referrer, latency_ms FROM page_views WHERE ip = %s ORDER BY created_at ASC",
        (ip,))
    conn.close()
...
def api_traffic():
    ...
    total_visitors = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s", (since,)) or {}).get("c", 0)
    total_views = (_one(conn, "SELECT COUNT(*) as c FROM page_views WHERE created_at >= %s", (since,)) or {}).get("c", 0)
    avg_per_day = round(total_visitors / days, 1)

    # Previous period for comparison
    prev_visitors = (_one(conn, "SELECT COUNT(DISTINCT ip) as c FROM page_views WHERE created_at >= %s AND created_at < %s", (prev_since, since)) or {}).get("c", 0)

    # Repeat visitors — seen on 2+ distinct days
    repeat_rows = _all(conn, """
        SELECT ip, COUNT(DISTINCT created_at::date) as days, COUNT(*) as hits
        FROM page_views WHERE created_at >= %s
        GROUP BY ip HAVING COUNT(DISTINCT created_at::date) >= 2
        ORDER BY days DESC, hits DESC LIMIT 10
    """, (since,))
    repeat_count = len(repeat_rows)

    # Daily breakdown
    daily_rows = _all(conn, """
        SELECT created_at::date as date, COUNT(DISTINCT ip) as v, COUNT(*) as hits
        FROM page_views WHERE created_at >= %s
    ...
    top_pages = _all(conn, """
        ...
        FROM page_views WHERE created_at >= %s AND path NOT LIKE '/az-cockpit%%' AND path NOT LIKE '/api/%%' AND path NOT LIKE '/static/%%'
    ...
    top_refs = _all(conn, """
        ...
        FROM page_views WHERE created_at >= %s AND referrer != '' AND referrer IS NOT NULL
    ...
    entry_rows = _all(conn, """
        SELECT path, COUNT(*) as sessions FROM (
            SELECT ip, created_at::date as day, MIN(created_at) as first_hit, path
            FROM page_views WHERE created_at >= %s
            GROUP BY ip, created_at::date, path
        ) sub GROUP BY path ORDER BY sessions DESC LIMIT 10
    """, (since,))

    # High engagement visitors (5+ distinct pages)
    engaged_rows = _all(conn, """
        SELECT ip, COUNT(DISTINCT path) as pages, COUNT(*) as hits,
               MAX(created_at) as last_seen, MIN(path) as first_path
        FROM page_views WHERE created_at >= %s
        GROUP BY ip HAVING COUNT(DISTINCT path) >= 5
