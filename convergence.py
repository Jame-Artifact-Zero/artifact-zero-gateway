"""
convergence.py — Core Pipeline Convergence Engine

Architecture (INV-15): Human → V2 → AI → V3 → Human
Convergence: Iterative V2/V3 passes until structural integrity meets threshold.

v1.5 Spec Mapping:
  - Maximum loop count:        DEFAULT_MAX_ITER (3). Core Plus: configurable up to 5.
  - Convergence criteria:      V2 score >= threshold (DEFAULT_THRESHOLD 0.80).
  - Cost ceiling:              DEFAULT_MAX_TOKENS_V3 (400). Halts if exceeded.
  - Deterministic stopping:    If V2 score meets threshold, stop immediately.
  - Escalation cutoff:         Loop exhausts max_iter, returns best-effort output.
  - Management routing:        If V2 route == HUMAN_INTERNAL, skip AI entirely (INV-15 Pre-AI loop).
  - Shadow mode:               Returns raw AI output to user but logs stabilized version for review.
  - Fail-safe:                 On error, falls back to raw AI call. Never breaks production.

Core vs Core Plus:
  - Core:       1 pass (max_iter=1), no iteration, V3 optional.
  - Core Plus:  Configurable (max_iter=3-5), V3 enabled, custom thresholds.
"""
