/**
 * az-shell.js v3.0 — THE LAST TIME
 * Unified header + footer for all Artifact Zero pages.
 *
 * v3.0:
 * - STICKY header (not fixed) — stays in document flow, no spacer needed
 * - svh viewport units with vh fallback for mobile Safari/Chrome
 * - Footer at bottom via flex column body
 * - Mobile hamburger at 700px
 * - Removes orphaned .footer/.topbar/.nav from templates
 * - Removes old v2 spacer divs
 * - Zero flicker: templates are content-only, shell provides all chrome
 */
(function(){
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    s.textContent = [
      ...
      'body{min-height:100%;min-height:100svh;display:flex;flex-direction:column;margin:0}',
      '.az-shell-content-wrap{flex:1 0 auto}',

      '.az-topbar{position:sticky;top:0;z-index:9000;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;background:rgba(10,12,16,.97);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border-bottom:1px solid rgba(37,42,53,.5)}',
  ...
  function removeExisting(){
    ['.topbar','.nav','.header-bar','header.topbar','[class*="topbar"]'].forEach(function(sel){
      document.querySelectorAll(sel).forEach(function(el){
        if(!el.classList.contains('az-topbar') && !el.classList.contains('az-footer')) el.remove();
      });
    });
    document.querySelectorAll('.footer').forEach(function(el){
      if(!el.classList.contains('az-footer')) el.remove();
    });
    document.querySelectorAll('.az-topbar-spacer').forEach(function(el){ el.remove(); });
  }
  ...
  function buildTopbar(){
    ...
    NAV_LINKS.forEach(function(item){
      ...
      if(path === item.href || (item.href !== '/' && path.startsWith(item.href))) a.className = 'active';
      nav.appendChild(a);
    });
    bar.appendChild(nav);
  ...
  function buildFooter(){
    ...
    [{label:'SafeCheck',href:'/safecheck'},{label:'API',href:'/docs'},{label:'Examples',href:'/examples'},{label:'Contact',href:'/contact'}].forEach(function(link){
      var a = document.createElement('a');
      a.href = link.href;
  ...
  function wrapContent(){
    ...
    while(node && node !== footer){ nodes.push(node); node = node.nextSibling; }
  ...
  function loadCockpit(){
    fetch('/api/cockpit/config').then(function(r){ return r.json(); }).then(function(cfg){
      if(cfg.banner && cfg.banner.on && cfg.banner.text){
        ...
        var topbar = document.querySelector('.az-topbar');
        if(topbar && topbar.nextSibling) document.body.insertBefore(b, topbar.nextSibling);
      }
      ...
      if(cfg.copy){
        if(cfg.copy.hero_h1){ var h1=document.querySelector('h1'); if(h1) h1.textContent=cfg.copy.hero_h1; }
        if(cfg.copy.hero_sub){ var sub=document.querySelector('.subline,.subtitle,.hero-sub,h2'); if(sub) sub.textContent=cfg.copy.hero_sub; }
        if(cfg.copy.cta_btn){ var cta=document.querySelector('.score-button,.cta-button,button[type="submit"]'); if(cta) cta.textContent=cfg.copy.cta_btn; }
        if(cfg.copy.custom_selector && cfg.copy.custom_value){ var el=document.querySelector(cfg.copy.custom_selector); if(el) el.textContent=cfg.copy.custom_value; }
      }
    }).catch(function(){});
  }
